...
def FastNDS2(fitness, ps):
    ...
    for i in range(L):
        for j in range(L):
            ...
            if dom_less == 0 and dom_equal != 2: # i 被 j 支配
                S[i].n = S[i].n + 1;
            elif dom_more == 0 and dom_equal != 2: # i 支配 j
                S[i].p.append(j)
        if S[i].n == 0: # 将 i 加入 Pareto 前沿
            F[rank].f.append(i)
    # 找到其他前沿
    while len(F[rank].f) > 0:
        Q = []
            ...
            if len(S[x].p) > 0:
                ...
                for j in range(pL):
                    ...
                    if S[k].n == 0:
                        Q.append(k)
        rank = rank + 1
        F.append(front())  # 创建新的前沿
        F[rank].f = Q

    # 拥挤度距离策略
    fL = len(F)
    fL = fL - 1  # 最后一个前沿是空的
    obj = [row[:2] for row in fitness]
    currentindex = 0
    for Frt in range(fL):
        ffL = len(F[Frt].f)
        y = np.zeros(shape=(ffL, 2))
        ...
        crowd[:, 0] = crowd[:, 0] + crowd[:, 1]
        tmp = []
        sort_index = np.argsort(-crowd[:, 0])  # 加 '-' 表示降序排序
        l = len(sort_index)
        for i in range(l):
            tmp.append(F[Frt].f[i])
        for i in range(l):
